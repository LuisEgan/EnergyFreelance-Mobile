...
export const authUserSuccess = (response) => {
  return {
    type: types.AUTH_BY_TYPE,
    payload: { response },
  };
};

export const authUserFailure = (status, response) => {
  return { type: types.AUTH_FAILURE, payload: { status, response } };
};

export const authByTypeSuccess = (response, actionPayload) => ({
  type: types.AUTH_BY_TYPE_SUCCESS,
  payload: { response, actionPayload },
});

export const authByTypeFailure = (status, response) => ({
  type: types.AUTH_BY_TYPE_FAILURE,
  payload: { status, response },
});
